...
use crate::{InfoHash, TorrentError, TorrentPeer, TorrentTracker, MAX_SCRAPE_TORRENTS};
use aquatic_udp_protocol::{
    AnnounceInterval, AnnounceRequest, AnnounceResponse, ConnectRequest, ConnectResponse,
    ErrorResponse, NumberOfDownloads, NumberOfPeers, Port, Request, Response, ResponsePeer,
...
pub async fn handle_packet(
    ...
    match Request::from_bytes(&payload[..payload.len()], MAX_SCRAPE_TORRENTS)
...
pub async fn handle_scrape(
    ...
    for info_hash in request.info_hashes.iter() {
        ...
        if authenticate(&info_hash, tracker.clone()).await.is_err() {
            continue;
        }
