...
use crate::{InfoHash, TorrentEntry, AUTH_KEY_LENGTH};
use log::debug;
use r2d2::Pool;
...
impl SqliteDatabase {
    ...
    pub fn create_database_tables(
        ...
        match conn.execute(&create_whitelist_table, NO_PARAMS) {
            Ok(updated) => match conn.execute(&create_keys_table, NO_PARAMS) {
                Ok(updated2) => match conn.execute(&create_torrents_table, NO_PARAMS) {
    ...
    pub async fn load_persistent_torrent_data(
        &self,
    ) -> Result<Vec<(InfoHash, u32)>, rusqlite::Error> {
    ...
    pub async fn save_persistent_torrent_data(
        &self,
        ...
        for (info_hash, torrent_entry) in torrents {
            ...
            let _ = db_transaction.execute(
                "INSERT OR REPLACE INTO torrents (info_hash, completed) VALUES (?, ?)",
