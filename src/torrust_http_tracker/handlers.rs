...
use crate::torrust_http_tracker::{
    AnnounceRequest, AnnounceResponse, ErrorResponse, Peer, ScrapeRequest, ScrapeResponse,
    ScrapeResponseEntry, ServerError, WebResult,
};
...
pub async fn handle_announce(
    ...
    let peers = tracker
        .get_torrent_peers(&announce_request.info_hash, &peer.peer_addr)
        .await;
    ...
    send_announce_response(
        ...
        peers,
        announce_interval,
        tracker.config.announce_interval_min,
    )
}
...
pub async fn handle_scrape(
    ...
    for info_hash in scrape_request.info_hashes.iter() {
        ...
        if authenticate(info_hash, &auth_key, tracker.clone())
            .await
            .is_err()
        {
            continue;
