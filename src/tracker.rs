...
use crate::{key_manager, Configuration, MAX_SCRAPE_TORRENTS};
...
impl TorrentPeer {
    pub fn from_udp_announce_request(
        ...
        let peer_addr = TorrentPeer::peer_addr_from_ip_and_port_and_opt_host_ip(
            remote_ip,
            host_opt_ip,
            announce_request.port.0,
        );
    ...
    pub fn peer_addr_from_ip_and_port_and_opt_host_ip(
        remote_ip: IpAddr,
        host_opt_ip: Option<IpAddr>,
        port: u16,
    ) -> SocketAddr {
    ...
    pub fn get_peers(&self, remote_addr: Option<&std::net::SocketAddr>) -> Vec<TorrentPeer> {
        ...
        for (_, peer) in self
            ...
            .filter(|e| match remote_addr {
                ...
                Some(remote_address) => match e.1.peer_addr.ip() {
...
impl TorrentTracker {
    ...
    fn is_private(&self) -> bool {
        self.config.mode == TrackerMode::PrivateMode
            || self.config.mode == TrackerMode::PrivateListedMode
    }

    fn is_whitelisted(&self) -> bool {
        self.config.mode == TrackerMode::ListedMode
            || self.config.mode == TrackerMode::PrivateListedMode
    }
    ...
    pub async fn get_torrent_peers(
        ...
        match read_lock.get(info_hash) {
            None => vec![],
            Some(entry) => entry.get_peers(Some(peer_addr)),
        }
    }
    ...
    pub async fn add_torrent(
        ...
        torrents.entry(info_hash).or_insert_with(|| {
        ...
        for (k, torrent_entry) in db.iter_mut() {
            // timed-out peers..
            {
                ...
                for (peer_id, peer) in torrent_peers.iter() {
                    if peer.is_seeder() {
                        if peer.updated.elapsed()
                            > std::time::Duration::from_secs(self.config.peer_timeout as u64)
                        {
                            // remove seeders after 5 minutes since last update...
                            peers_to_remove.push(peer_id.clone());
                            torrent_entry.seeders -= 1;
                        }
                    } else if peer.updated.elapsed()
                        > std::time::Duration::from_secs(self.config.peer_timeout as u64)
                    {
            ...
            if self.config.mode.clone() == TrackerMode::PublicMode
                && self.config.cleanup_peerless
                && !self.config.persistence
            {
                // peer-less torrents..
                if torrent_entry.peers.is_empty() {
                    torrents_to_remove.push(*k);
                }
            }
        }
